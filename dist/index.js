...
// src/utils.ts
import { writeFile as nodeWriteFile, unlink as nodeUnlink, access as nodeAccess } from "fs/promises";
...
function safeSpawn(command, args, opts = {}) {
  const { timeout = 60000, env, cwd, signal } = opts;
...
async function toolExists(name) {
  try {
    await safeSpawn("which", [name], { timeout: 5000 });
    return true;
  } catch {
    return false;
  }
...
async function detectAudioTool() {
  ...
  throw new Error("No audio capture tool found. Install one of: ffmpeg (recommended), sox, or parecord (Linux). See README for installation instructions.");
}
...
class FfmpegRecorder {
  ...
  async start(audioDevice) {
    ...
    this.child = spawn2(args[0], args.slice(1), {
      detached: false,
      env: process.env
    });
...
function buildArgs(tool, outputPath, device) {
  const pf = platform();
  switch (tool) {
    case "ffmpeg": {
    ...
    default: {
      throw new Error(`Unsupported audio tool: ${tool}`);
    }
  }
}
...
_AssistantStream_addEvent = function _AssistantStream_addEvent2(event) {
  ...
  switch (event.event) {
    ...
    case "thread.message.completed":
    case "thread.message.incomplete":
      if (__classPrivateFieldGet4(this, _AssistantStream_currentContentIndex, "f") !== undefined) {
...
class OpenAiWhisperProvider {
  ...
  async transcribe(audioPath, signal) {
    ...
    if (info.size > MAX_FILE_SIZE_BYTES) {
      throw new Error(`Audio file is ${(info.size / 1024 / 1024).toFixed(1)}MB, exceeds OpenAI's 25MB limit.`);
    }
    if (info.size === 0) {
...
function getConfig(kv) {
  return {
    apiKey: kv.get(KV_KEYS.apiKey, DEFAULTS.apiKey) || DEFAULTS.apiKey,
    model: kv.get(KV_KEYS.model, DEFAULTS.model) || DEFAULTS.model,
    audioDevice: kv.get(KV_KEYS.audioDevice, DEFAULTS.audioDevice) || DEFAULTS.audioDevice,
    customPrompt: kv.get(KV_KEYS.customPrompt, DEFAULTS.customPrompt) || DEFAULTS.customPrompt,
    showRecordingToast: kv.get(KV_KEYS.showRecordingToast, DEFAULTS.showRecordingToast),
    showTranscriptionToast: kv.get(KV_KEYS.showTranscriptionToast, DEFAULTS.showTranscriptionToast)
  };
}
function setConfig(kv, patch) {
  for (const [key, value] of Object.entries(patch)) {
    if (value === undefined) {
      kv.set(KV_KEYS[key], DEFAULTS[key]);
    } else {
      kv.set(KV_KEYS[key], value);
    }
  }
}
...
var plugin = {
  ...
  tui: async (api) => {
    ...
    function startRecordingIndicator() {
      ...
      if (!cfg.showRecordingToast)
        return;
      let dotCount = 0;
      const dots = ["\xB7  ", "\xB7\xB7 ", "\xB7\xB7\xB7"];
      toast({ variant: "info", title: "\uD83C\uDFA4 Recording", message: "Listening " + dots[0], duration: 60000 });
      recordingIndicator = setInterval(() => {
        dotCount = (dotCount + 1) % dots.length;
        toast({ variant: "info", title: "\uD83C\uDFA4 Recording", message: "Listening " + dots[dotCount], duration: 60000 });
      }, 500);
    }
    ...
    function openSettingsMenu(kv2, toast2, onChange) {
      const cfg = getConfig(kv2);
        ...
        options: [
          {
            ...
            onSelect: () => {
              api.ui.dialog.replace(() => api.ui.DialogPrompt({
                ...
                value: cfg.apiKey,
                onConfirm: (value) => {
                  setConfig(kv2, { apiKey: value.trim() || undefined });
                  onChange();
                  toast2({ variant: "success", title: "Saved", message: "API key updated.", duration: 2000 });
                  api.ui.dialog.clear();
                },
          ...
          },
          {
            ...
            onSelect: () => {
              api.ui.dialog.replace(() => api.ui.DialogPrompt({
                ...
                onConfirm: (value) => {
                  setConfig(kv2, { audioDevice: value.trim() || undefined });
                  onChange();
                  toast2({ variant: "success", title: "Saved", message: "Audio device updated.", duration: 2000 });
                  api.ui.dialog.clear();
                },
                onCancel: () => api.ui.dialog.clear()
              }));
            ...
            onSelect: () => {
              ...
              api.ui.dialog.replace(() => api.ui.DialogSelect({
                ...
                options: models.map((m) => ({
                  ...
                  value: m,
                  onSelect: () => {
                    setConfig(kv2, { model: m });
                    onChange();
                    toast2({ variant: "success", title: "Saved", message: `Model: ${m}`, duration: 2000 });
                    api.ui.dialog.clear();
                  }
                }))
              }));
          ...
          },
          {
            ...
            onSelect: () => {
              api.ui.dialog.replace(() => api.ui.DialogPrompt({
                ...
                onConfirm: (value) => {
                  setConfig(kv2, { customPrompt: value.trim() || undefined });
                  toast2({ variant: "success", title: "Saved", message: "Prompt updated.", duration: 2000 });
                  api.ui.dialog.clear();
                },
                onCancel: () => api.ui.dialog.clear()
              }));
          ...
          },
          {
            ...
            onSelect: () => {
              ...
              setConfig(kv2, { showRecordingToast: next });
              toast2({ variant: "success", title: "Saved", message: `Recording indicator: ${next ? "on" : "off"}`, duration: 2000 });
              api.ui.dialog.clear();
            }
          },
          {
            ...
            onSelect: () => {
              ...
              setConfig(kv2, { showTranscriptionToast: next });
              toast2({ variant: "success", title: "Saved", message: `Transcription toast: ${next ? "on" : "off"}`, duration: 2000 });
              api.ui.dialog.clear();
            }
          },
          {
            ...
            onSelect: () => {
              api.ui.dialog.replace(() => api.ui.DialogConfirm({
                ...
                onConfirm: () => {
                  setConfig(kv2, { apiKey: undefined, model: undefined, audioDevice: undefined, customPrompt: undefined, showRecordingToast: undefined, showTranscriptionToast: undefined });
                  onChange();
                  toast2({ variant: "success", title: "Reset", message: "Settings restored to defaults.", duration: 2000 });
                  api.ui.dialog.clear();
                },
                onCancel: () => api.ui.dialog.clear()
              }));
...
var id = plugin.id;
var tui = plugin.tui;
var opencode_talk_default = plugin;
export {
  tui,
  id,
  opencode_talk_default as default
};
